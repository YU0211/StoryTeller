...
LANGUAGE_ALL = {
    'zh_CN': {
        ...
        'LANGUAGE': 'zh_CN',
        '選擇模型文件': '選擇模型文件',
        '模型：.pt格式(自動識別同目錄下config.yaml)': '模型：.pt格式(自動識別同目錄下config.yaml)',
    ...
    'en_US': {
        ...
        '模型：.pt格式(自動識別同目錄下config.yaml)': 'Model：.pt format(Auto ust config.yaml in here)',
    ...
    'ja_JP': {
        ...
        'LANGUAGE': 'ja_JP',
        '選擇模型文件': 'モデルを選択',
        '模型：.pt格式(自動識別同目錄下config.yaml)': 'モデル：.pt形式（同じディレクトリにあるconfig.yamlを自動認識します）',
