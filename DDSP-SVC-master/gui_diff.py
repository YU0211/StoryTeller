...
class Config:
    def __init__(self) -> None:
        ...
        self.spk_id = 1  # 默認說話人。
        self.spk_mix_dict = None  # {1:0.5, 2:0.5} 表示1號說話人和2號說話人的音色按照0.5:0.5的比例混合
        self.use_phase_vocoder = True
        self.checkpoint_path = ''
        self.threhold = -35
        self.buffer_num = 2
        self.crossfade_time = 0.03
        self.select_pitch_extractor = 'harvest'  # F0預測器["parselmouth", "dio", "harvest", "crepe", "rmvpe", "fcpe"]
...
class GUI:
    def __init__(self) -> None:
        self.config = Config()
        self.flag_vc: bool = False  # 變聲執行緒flag
        self.block_frame = 0
        self.crossfade_frame = 0
        self.sola_search_frame = 0
        self.device = 'cuda' if torch.cuda.is_available() else 'cpu'
        self.svc_model: SvcDDSP = SvcDDSP()
        self.fade_in_window: np.ndarray = None  # crossfade計算用numpy數組
        self.fade_out_window: np.ndarray = None  # crossfade計算用numpy數組
        self.input_wav: np.ndarray = None  # 輸入音訊規範化後的保存地址
        self.output_wav: np.ndarray = None  # 輸出音訊規範化後的保存地址
        self.sola_buffer: torch.Tensor = None  # 保存上一個output的crossfade
        self.f0_mode_list = ["parselmouth", "dio", "harvest", "crepe" ,"rmvpe", "fcpe"]  # F0預測器
        self.diff_method_list = ["ddim", "pndm", "dpm-solver", "unipc"] # 加速採樣方法
    ...
    def launcher(self):
        ...
        sg.theme('DarkBlue12')  # 設置主題
        # 界面布局
        layout = [
            [sg.Frame(layout=[
                [sg.Input(key='sg_model', default_text=''),
                 sg.FileBrowse(i18n('選擇模型文件'), key='choose_model')]
            ], title=i18n('DDSP模型（4.0版本及以後可以不需要）:')),
                sg.Frame(layout=[
                    [sg.Text(i18n('選擇配置文件所在目錄')), sg.Input(key='config_file_dir', default_text='exp'),
                     sg.FolderBrowse(i18n('打開文件夾'), key='choose_config')],
                    [sg.Button(i18n('讀取配置文件'), key='load_config'),
            ...
            [sg.Frame(layout=[
                [sg.Text(i18n("說話人id")), sg.Input(key='spk_id', default_text='1', size=8)],
                [sg.Text(i18n("響應閾值")),
                ...
                [sg.Text(i18n("變調")),
                 sg.Slider(range=(-24, 24), orientation='h', key='pitch', resolution=1, default_value=0,
                           enable_events=True)],
                [sg.Text(i18n("採樣率")), sg.Input(key='samplerate', default_text='44100', size=8)],
                [sg.Checkbox(text=i18n('啟用捏音色功能'), default=False, key='spk_mix', enable_events=True),
                 sg.Button(i18n("設置混合音色"), key='set_spk_mix')]
            ], title=i18n('普通設置')),
                sg.Frame(layout=[
                    [sg.Text(i18n("音訊切分大小")),
                    ...
                    [sg.Text(i18n("f0預測模式")),
                     sg.Combo(values=self.f0_mode_list, key='f0_mode', default_value=self.f0_mode_list[-1],
                              enable_events=True)],
                    [sg.Checkbox(text=i18n('啟用相位聲碼器'), default=False, key='use_phase_vocoder',
                                 enable_events=True)]
                ], title=i18n('性能設置')),
                sg.Frame(layout=[
                    [sg.Text(i18n("擴散模型文件：.pt格式(自動識別同目錄下config.yaml)"))],
                    [sg.Input(key='diff_project', default_text='exp\\diffusion-new-demo\\model_200000.pt'),
                     sg.FileBrowse(i18n('選擇模型文件'), key='choose_model')],
                    [sg.Text(i18n("擴散深度")), sg.Input(key='k_step', default_text='100', size=18)],
                    [sg.Text(i18n("擴散加速")), sg.Input(key='diff_acc', default_text='10', size=18)],
                    [sg.Text(i18n("擴散算法")),
                     sg.Combo(values=self.diff_method_list, key='diff_method', default_value=self.diff_method_list[0],
                              enable_events=True)],
                    [sg.Checkbox(text=i18n('不擴散安全區(加速但損失效果)'), default=False, key='diff_silence', enable_events=True)]
                ], title=i18n('擴散設置')),
            ],
            [sg.Button(i18n("開始音訊轉換"), key="start_vc"), sg.Button(i18n("停止音訊轉換"), key="stop_vc"),
             sg.Text(i18n('推理所用時間(ms):')), sg.Text('0', key='infer_time')]
        ]
    ...
    def event_handler(self):
        ...
        while True:  # 事件處理循環
            ...
            if event == sg.WINDOW_CLOSED:  # 如果用戶關閉窗口
                self.flag_vc = False
                exit()
            ...
            if event == 'start_vc' and self.flag_vc == False:
                ...
                spk_mix = sg.popup_get_text(message='範例：1:0.3,2:0.5,3:0.2', title="設置混合音色，支持多人")
                if spk_mix != None:
